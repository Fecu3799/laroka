...
// Se monta siempre desde History con open={!!shift}: mientras el panel está
// abierto, cambiar de turno sólo re-renderiza el contenido — no desmonta el
// Drawer, así que no se repite la animación de entrada.
export default function ShiftDetailDrawer({ shift, open, onClose }) {
  const { openedAt, closedAt, openedBy, summary } = shift ?? {}

  return (
    <Drawer
      open={open}
      onClose={onClose}
      title={`Turno #${formatShiftStamp(openedAt)}`}
      subtitle={formatShiftDate(openedAt)}
      icon={<ClockIcon />}
