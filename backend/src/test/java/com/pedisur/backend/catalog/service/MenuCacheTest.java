...
class MenuCacheTest {
	...
	void update_evictsMenuCache() {
		...
		com.pedisur.backend.catalog.entity.Product existing = initial.get(0).getProduct();
		clearInvocations(branchProductRepository);
		...
		com.pedisur.backend.catalog.entity.Product updates = com.pedisur.backend.catalog.entity.Product.builder()
			.name(existing.getName())
			.description(existing.getDescription())
			.price(existing.getPrice())
			.imageUrl("https://cdn.r2.dev/1/products/updated.png")
			.category(com.pedisur.backend.catalog.entity.Category.builder().id(1).build())
			.tenant(com.pedisur.backend.tenant.entity.Tenant.builder().id(1).build())
			.build();
